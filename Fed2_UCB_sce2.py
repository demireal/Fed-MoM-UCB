...
def gp(p):
    gp = 20  # choose this so that lambda makes sense for practical reasons, i.e., if lambda = 0.25 and gp = 10, we cant have 2.5 Byzantine clients recruited.
    return int(gp)


def Byz_sample(m, k, optimal_indices, mu_local, sigma):
    if k in optimal_indices:
        obs = np.random.rand(1)[0]/10  # [0, 0.1]
    else:
        obs = np.random.normal(mu_local[m,k],sigma)
    return obs
...
K = len(mu_global)
lam = 0.25 # Byzantine fraction (can change to 0.25)
global T
T = int(1e5)
N = 100  # repeat times
sigma = 1/2  # arm noise
...
for rep in range(N):
    ...
    while t < T:
        ...
        '''
        if len(active_arm) > 1:
            print('n = {}, t = {}, p = {}, Active Arms: {}'.format(rep, t, p, active_arm), 20* ' ', end='\r')
            player_add_num = gp(p)
            if M==0:
                M += 1
        ...
        if len(active_arm) > 1:
            for k in active_arm:
                for _ in range(min(T-t,expl_len)):
                    for m in range(M):
                        if m in Byz_indices:
                            reward_local[m,k] += Byz_sample(m, k, optimal_indices, mu_local, sigma)  # arm sampling
                        else:
...
np.save('./np_arrays/mean_fucb_sce_{}_regret_sp10p_mp20_lam_{}.npy'.format(scenario, lam), np.mean(regret, axis=0))
np.save('./np_arrays/std_fucb_sce_{}_regret_sp10p_mp20_lam_{}.npy'.format(scenario, lam), np.std(regret, axis=0))
np.save('./np_arrays/mean_fucb_sce_{}_comm_regret_sp10p_mp20_lam_{}.npy'.format(scenario, lam), np.mean(comm_regret, axis=0))
np.save('./np_arrays/std_fucb_sce_{}_comm_regret_sp10p_mp20_lam_{}.npy'.format(scenario, lam), np.std(comm_regret, axis=0))
np.save('./np_arrays/mean_fucb_sce_{}_nocomm_regret_sp10p_mp20_lam_{}.npy'.format(scenario, lam), np.mean(no_comm_reg, axis=0))
np.save('./np_arrays/std_fucb_sce_{}_nocomm_regret_sp10p_mp20_lam_{}.npy'.format(scenario, lam), np.std(no_comm_reg, axis=0))
np.save('./np_arrays/fucb_sce_{}_M_arr_sp10p_mp20_lam_{}.npy'.format(scenario, lam), M_arr)
